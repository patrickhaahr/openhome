...
import {
  parseRadiusMeters,
  type HomeGeofence,
  type HomeGeofenceProvider,
} from "../domain/home-geofence";
...
export function useHomeGeofence(
  service: HomeGeofenceService,
): readonly [HomeGeofenceState, HomeGeofenceActions] {
  ...
  async function setHome(provider: HomeGeofenceProvider): Promise<void> {
    ...
    dispatch(
      result.ok ? { type: "saved", home: result.value } : { type: "failed", message: result.error },
    );
  }
  ...
  return [
    ...
    {
      updateRadius(value) {
        dispatch({ type: "radiusChanged", value });
      },
      setHome(provider) {
        void setHome(provider);
      },
      disable() {
        void disable();
      },
    ...
    case "saved":
      return state.tag === "ready"
        ? {
            ...state,
            home: event.home,
            provider: event.home.provider,
            radiusInput: event.home.radiusMeters.toString(),
            saving: false,
            error: null,
          }
        : state;
