...
type Event =
  | { readonly type: 'configurationLoaded'; readonly configuration: Configuration | null; readonly error: string | null }
  ...
  | { readonly type: 'commandFinished'; readonly target: CommandTarget; readonly command: string; readonly error: string | null };
...
export function useOpenHome(store: ConfigurationStore): readonly [OpenHomeState, OpenHomeActions] {
  ...
  async function refreshIr(api: OpenHomeApi): Promise<void> {
    ...
    dispatch(result.ok ? { type: 'irLoaded', status: result.value } : { type: 'irFailed', message: result.error });
  }
  ...
  function sendIr(target: 'edifier' | 'television', remote: 'edifier' | 'lgtv', command: string): void {
    const current = stateRef.current;
    if (current.tag !== 'ready' || current.ir.tag !== 'loaded') {
      return;
    }
    const available = remote === 'edifier' ? current.ir.status.edifierCommands : current.ir.status.lgTvCommands;
    if (!available.has(command) || current[target].sending.has(command)) {
      return;
    }
    void runCommand(target, command, createOpenHomeApi(current.configuration).sendIrCommand(remote, command));
  }
  ...
  function sendLight(command: 'on' | 'off'): void {
    ...
    void runCommand('light', command, createOpenHomeApi(current.configuration).sendLightCommand(command));
  }

  async function runCommand(target: CommandTarget, command: string, operation: Promise<Result<void>>): Promise<void> {
  ...
  const actions: OpenHomeActions = {
    updateBaseUrl(value) { dispatch({ type: 'baseUrlChanged', value }); },
    updateApiKey(value) { dispatch({ type: 'apiKeyChanged', value }); },
    submitConfiguration() { void saveConfiguration(); },
    openReconfiguration() {
      generation.current += 1;
    ...
    sendEdifierCommand(command) { sendIr('edifier', 'edifier', command); },
    sendTelevisionCommand(command) { sendIr('television', 'lgtv', command); },
...
export function reduce(state: OpenHomeState, event: Event): OpenHomeState {
  switch (event.type) {
    ...
    case 'baseUrlChanged':
      return state.tag === 'configuration' ? { ...state, baseUrl: event.value, error: null } : state;
    case 'apiKeyChanged':
    ...
    case 'configurationFailed':
      return state.tag === 'configuration' ? { ...state, isSaving: false, error: event.message } : state;
    case 'configurationSaved':
      return readyState(event.configuration, state.tag === 'configuration' && state.previous !== null ? state.previous.selectedTab : 'home');
    case 'reconfigurationOpened':
      return state.tag === 'ready'
        ? configurationState('reconfigure', withoutPendingCommands(state), state.configuration.baseUrl, state.configuration.apiKey, null)
        : state;
    ...
    case 'irLoaded':
      return state.tag === 'ready' ? { ...state, ir: { tag: 'loaded', status: event.status } } : state;
    case 'irFailed':
      return state.tag === 'ready' ? { ...state, ir: { tag: 'error', message: event.message } } : state;
    case 'commandStarted':
      return updateCommands(state, event.target, (current) => ({ sending: new Set([...current.sending, event.command]), error: null }));
    case 'commandFinished':
      return updateCommands(state, event.target, (current) => ({
        sending: new Set([...current.sending].filter((command) => command !== event.command)),
        error: event.error === null ? current.error : { command: event.command, message: event.error },
      }));
  }
}
