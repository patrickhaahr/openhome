    ...
    <View onLayout={handleLayout} style={styles.screen}>
      <View style={styles.header}>
        <View>
          ...
          <View
            accessibilityLabel={connectionLabel(state.ir)}
          ...
          >
            <View
              style={[
                ...
                state.ir.tag === "loaded" && styles.readyDot,
                state.ir.tag === "error" && styles.errorDot,
              ]}
            />
      ...
      >
        ...
        <Page width={pageWidth}>
          <TelevisionPage state={state} actions={actions} />
        </Page>
        <Page width={pageWidth}>
          <SpeakerPage state={state} actions={actions} />
        </Page>
        <Page width={pageWidth}>
      ...
      <View accessibilityRole="tablist" style={styles.tabs}>
        ...
        <Tab
          icon="television"
          label="TV"
          selected={state.selectedTab === "television"}
          onPress={() => selectTab("television")}
        />
        <Tab
          icon="speaker"
          label="Speaker"
          selected={state.selectedTab === "speaker"}
          onPress={() => selectTab("speaker")}
        />
        <Tab
          icon="away"
          label="Away"
  ...
  return (
    <View style={styles.stack}>
      <PageHeading
        ...
        title="Home"
        description="Your most-used controls, ready when you are."
      />
      {state.ir.tag !== "loaded" ? (
        <IrSummary state={state.ir} onRetry={actions.retryIrStatus} />
      ) : null}
        ...
        <CompactDeviceSection icon="light" title="Lights">
          <View style={styles.row}>
            <CompactAction
              ...
              disabled={state.light.sending.size > 0}
              onPress={() => actions.sendLightCommand("on")}
            />
            <CompactAction
              label="Off"
              sending={state.light.sending.has("off")}
              disabled={state.light.sending.size > 0}
              onPress={() => actions.sendLightCommand("off")}
            />
          </View>
          <CommandError
            state={state.light}
            commands={[
              { command: "on", label: "Lights on" },
              { command: "off", label: "Lights off" },
            ]}
          />
        </CompactDeviceSection>
      </View>
  ...
  return (
    <View style={styles.stack}>
      <PageHeading
        eyebrow="REMOTE"
        title="TV"
        description="Power, sound, channels, and navigation."
      />
      {state.ir.tag !== "loaded" ? (
        <IrSummary state={state.ir} onRetry={actions.retryIrStatus} />
      ) : null}
  ...
  return (
    <View style={styles.stack}>
      <PageHeading
        eyebrow="REMOTE"
        title="Speaker"
        description="Choose a source and shape the sound."
      />
      {state.ir.tag !== "loaded" ? (
        <IrSummary state={state.ir} onRetry={actions.retryIrStatus} />
      ) : null}
...
function AwayPage({
  ...
  readonly state: HomeGeofenceState;
  ...
  return (
    <View style={styles.stack}>
      <PageHeading
        eyebrow="AUTOMATION"
        title="Leave home"
        description="Turn off the lights when this device leaves home."
      />
          ...
          <>
            ...
            <View style={styles.statusCopy}>
              <Text style={styles.statusLabel}>
                {enabled
                  ? `Automation on with ${providerLabel(state.home?.provider)}`
                  : "Automation off"}
              </Text>
      ...
      <Section title="HOME REGION">
        <Text style={styles.automationDescription}>
          OpenHome uses your location only to detect when this device leaves the region. It then
        ...
        {state.tag === "loading" ? null : (
          <View style={styles.row}>
            ...
            />
            <SecondaryAction
              label={enabled ? "Update natively" : "Set natively"}
              disabled={state.saving}
              onPress={() => actions.setHome("native")}
            />
          </View>
        )}
        {enabled ? (
...
function AutomationToggle({
  ...
  return (
    <Pressable
      accessibilityHint={
        enabled
          ? "Turns off leave home automation"
          : `Sets this location as home using ${providerLabel(state.tag === "ready" ? state.provider : undefined)} location monitoring`
      }
...
function CompactDeviceSection({
  icon,
  title,
  children,
}: {
  readonly icon: "television" | "light";
...
function DeviceSection({
  ...
  readonly icon: "speaker" | "television";
  readonly title: string;
...
function RemoteRows({
  ...
  return (
    <View style={styles.rows}>
          ...
          {row.map((control, columnIndex) =>
            control === null ? (
              ...
              <ControlButton
                key={control.command}
                control={control}
                available={available.has(control.command)}
                sending={commands.sending.has(control.command)}
                onPress={onPress}
              />
            ),
          )}
      ...
      <CommandError
        state={commands}
        commands={rows.flat().filter((command): command is RemoteCommand => command !== null)}
      />
    </View>
  );
}
...
function CompactAction({
  ...
  return (
    <Pressable
      accessibilityLabel={`Turn lights ${label.toLowerCase()}`}
      accessibilityRole="button"
      accessibilityState={{ disabled, busy: sending }}
      disabled={disabled}
      onPress={onPress}
...
function CommandError({
  ...
  readonly commands: ReadonlyArray<RemoteCommand>;
}) {
  if (state.error === null) {
    return null;
  }
...
function Tab({
  ...
  readonly icon: Exclude<IconName, "settings">;
  readonly label: string;
  ...
  if (name === "television") {
    return (
      <View
        accessibilityElementsHidden
        importantForAccessibility="no"
        style={[styles.tvIcon, { borderColor: color, height: size * 0.72, width: size }]}
      >
        <View style={[styles.tvStand, { backgroundColor: color }]} />
      </View>
    );
  }
  if (name === "speaker") {
    return (
      <View
        accessibilityElementsHidden
        importantForAccessibility="no"
        style={[styles.speakerIcon, { borderColor: color, height: size, width: size * 0.72 }]}
      >
        <View style={[styles.speakerDriver, { borderColor: color }]} />
        <View style={[styles.speakerTweeter, { backgroundColor: color }]} />
      </View>
    );
  }
  if (name === "light") {
    return (
      <View
        accessibilityElementsHidden
        importantForAccessibility="no"
        style={[styles.bulbIcon, { height: size, width: size }]}
      >
