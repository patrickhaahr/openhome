    ...
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.screen}>
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.panel}>
          <Text style={styles.title}>{isReconfigure ? 'Update connection' : 'Connect your home'}</Text>
          ...
          <TextInput
            ...
            value={state.apiKey}
          />
          {state.error === null ? null : <Text accessibilityRole="alert" style={styles.error}>{state.error}</Text>}
          <View style={styles.actions}>
            {isReconfigure ? (
              <Pressable disabled={state.isSaving} onPress={actions.cancelReconfiguration} style={styles.secondaryButton}>
                <Text style={styles.secondaryLabel}>Cancel</Text>
              </Pressable>
            ) : null}
            <Pressable disabled={state.isSaving} onPress={actions.submitConfiguration} style={styles.primaryButton}>
              {state.isSaving ? <ActivityIndicator color={colors.background} /> : null}
              <Text style={styles.primaryLabel}>{state.isSaving ? 'Checking connection' : isReconfigure ? 'Save connection' : 'Connect'}</Text>
            </Pressable>
          </View>
        </View>
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: colors.background, borderColor: colors.border, borderRadius: 10, borderWidth: 1, color: colors.text, fontSize: 16, paddingHorizontal: 14, paddingVertical: 13 },
  ...
  panel: { alignSelf: 'center', backgroundColor: colors.panel, borderColor: colors.border, borderRadius: 18, borderWidth: 1, gap: 10, maxWidth: 520, padding: 22, width: '100%' },
  primaryButton: { alignItems: 'center', backgroundColor: colors.signal, borderRadius: 10, flex: 1, flexDirection: 'row', gap: 8, justifyContent: 'center', minHeight: 50, paddingHorizontal: 14 },
  primaryLabel: { color: colors.background, fontSize: 14, fontWeight: '800' },
  screen: { backgroundColor: colors.background, flex: 1 },
  secondaryButton: { alignItems: 'center', borderColor: colors.border, borderRadius: 10, borderWidth: 1, flex: 1, justifyContent: 'center', minHeight: 50 },
  ...
  wordmark: { color: colors.text, fontSize: 48, fontWeight: '900', letterSpacing: -2, lineHeight: 43, marginTop: 8 },
});
