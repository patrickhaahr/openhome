  ...
  return {
    async checkAvailability() {
      if (Platform.OS !== 'android' || nativeHomeGeofenceModule === null) {
        return failure('Native location is available only in an Android development or release build.');
      }
      try {
    ...
    async stop() {
      if (nativeHomeGeofenceModule !== null && await nativeHomeGeofenceModule.hasStartedMonitoringAsync(nativeHomeGeofenceTaskName)) {
