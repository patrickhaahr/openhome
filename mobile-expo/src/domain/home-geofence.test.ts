...
describe("parseRadiusMeters", () => {
  ...
  it("rejects unreliable and fractional distances", () => {
    expect(parseRadiusMeters("99")).toEqual({
      ok: false,
      error: "Enter a radius of at least 100 meters.",
    });
    expect(parseRadiusMeters("10.5")).toEqual({
      ok: false,
      error: "Enter a whole number of meters.",
    });
  });
});
